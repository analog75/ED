...
int main(int argc, char *argv[]) {
  ...
  else if (mode == DIAGONAL_PRUNGING_CHECK)
    evaluate_seq_diag_pruning_check(parameter);
...
{
  std::cout << "*** approximate string matching using diagonal " 
            << "sequential dynamic programming and array pruning are performed!! ***" 
            << std::endl;
  ...
  return;
} // end of void evaluate_seq_diag_pruning(Option& _parameter)

void evaluate_seq_diag_pruning_check(Option& _parameter)
{
  std::cout << "*** approximate string matching using diagonal " 
            << "sequential dynamic programming and pruning of each step are performed!! ***" 
            << std::endl;
  ...
  char **check = (char**)calloc(length_input+1, sizeof(char*));

  for (int i=0; i < length_input+1; i++)
  {
    check[i] = (char *)calloc(length_pattern+1, sizeof(char));
  }

  // row: pattern, column: input 
  std::chrono::nanoseconds elapsed_time;
  edit_distance_diag_pruning_check(input, pattern, check, length_input, length_pattern, k, dist_array, &elapsed_time);
  ...
  return;
} // end of void evaluate_seq_diag_pruning_check(Option& _parameter)
